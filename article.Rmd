...
knitr::opts_chunk$set(echo = TRUE, message=FALSE, fig.height=2, fig.align="left")
```
...
all.gpca %>% plot("ranks")
```
```{r gpca plot delta, fig.cap="\\label{figs:figs} Delta statistics shown on with cumulative parts of variance"}
all.gpca %>% plot("delta")
```
