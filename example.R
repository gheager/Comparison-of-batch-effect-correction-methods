...
cbind(geod,mtab)->whole; batch<-c('geod' %>% rep(ncol(geod)),'mtab' %>% rep(ncol(mtab))); tissue<-c(geod74747$organism_part,mtab4644$organism_part)
...
plot(v,p %>% abs)
ggplot()+aes(x=v,y=p %>% abs)+geom_point()+geom_smooth(method='lm')
...
filtered[,batch=='geod'] %>% t %>% prcomp(rank.=1) -> pcageod
filtered[,batch=='mtab'] %>% t %>% prcomp(rank.=1) -> pcamtab

angledet(pcageod$rotation,pcamtab$rotation)->alpha
...
angledet(pcageod$rotation,pcaall$rotation)->alpha
ggplot()+aes(x=pcageod$x,y=0,colour=tissue[batch=='geod'])+geom_point()+
